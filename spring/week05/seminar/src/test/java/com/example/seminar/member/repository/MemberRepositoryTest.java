...
class MemberRepositoryTest {
    ...
    void 이름과_최소_나이로_회원을_조회한다() {
        // when
        List<Member> result =
                memberRepository.findByUsernameContainingIgnoreCaseAndAgeGreaterThanEqual("민", 20);
        //then
        assertThat(result).extracting(Member::getUsername).containsExactlyInAnyOrder("민서","민지");
    }
    ...
    void 활성_회원을_페이징하여_조회한다() {
        ...
        // when
        Page<Member> result = memberRepository.findByActiveTrue(pageable);
    ...
    void JPQL로_활성_회원을_검색한다() {
        // when
        List<Member> result = memberRepository.searchByJpql("민",20);
        // then
        assertThat(result)
